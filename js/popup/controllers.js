	...
	class Controller {
		...
		constructor(scope, document) {
			...
			for (const func of [
				...
				//added function
				this.onClickRecommendHighlight,
				this.onClickUseful,
				this.onClickUseless,
				this.onClickSuggestHighlight,
		...
		onClickHighlight(doc) {		
			...
			return ChromeTabs.queryActiveTab().then(tab => {
				...
				return tabs.selectHighlight(doc._id).then(() => tabs.scrollToHighlight(doc._id)).then(() =>{ 
								var time = new Date(doc.date);
			  ...
			  var MKTIME = y+'-'+m+'-'+d+' '+h+':'+mm+':'+s;

        var MKID = doc._id;
        var MKITEM = tab.url;
        var MKKEYWORDS = doc[DB.DOCUMENT.NAME.TEXT];
        ...
        xhr.open('POST','http://192.168.153.133:80/insert.php',true);
        xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
        
        //5、发送请求
        var content = '&MKID='+MKID+'&MKITEM='+MKITEM+'&MKKEYWORDS='+MKKEYWORDS+'&MKTIME='+MKTIME;
        xhr.send(content); 
        console.log(content);
			})
      ...
      if (doc==""){
       ...
       if (window.XMLHttpRequest){
        ...
        xmlhttp.onreadystatechange=function(){
         ...
         xmlhttp.open("GET","http://192.168.153.133:80/recommend.php?q="+MKID,true);
         xmlhttp.send();
         
    }

	
	//onClickSuggestHighlight(doc) {
			//var str = this.scope.search.text;
			//console.log(str);	
			//confirm("Thank you for your suggestion");
			//if (r==true){
			//	x="你按下了\"确定\"按钮!";
			//}
			//else{
		...
		onClickSuggestHighlight(doc){
        //1、获取准备Post内容
        var MKSUGGESTION = $scope.suggest.text;
        var MKID = doc._id;
				console.log(MKSUGGESTION);	
				console.log(MKID);	
        ...
        //3、确定请求参数
        xhr.open('POST','http://192.168.153.133:80/suggest.php',true);
        xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
        //4、重写回调函数
        ...
        //5、发送请求
        var content = '&MKID='+MKID+'&MKSUGGESTION='+MKSUGGESTION;
        xhr.send(content); 
        confirm("Thank you for your suggestion");     
